...
def determine_if_diagonal(line):
    x_change = int(line['end'][0])-int(line['start'][0])
    y_change = int(line['end'][1]) - int(line['start'][1])
    gradient = float(x_change/y_change)
    if abs(gradient) == 1:
        return True
    else:
        return False
...
def determine_type_of_line(line):
    if line['start'][0] == line['end'][0]:
        ...
        return 'horizonatal'
    elif determine_if_diagonal(line):
        return 'diagonal'
    else:
        return "other"
...
def plot_diagonal_line(line, matrix):
    x_change = int(line['end'][0])-int(line['start'][0])
    y_change = int(line['end'][1]) - int(line['start'][1])
    diagonal_line_length = abs(x_change) +1  # include the end point
    x_start =int(line['start'][0])
    y_start = int(line['start'][1])
    for x in range(diagonal_line_length):
...
for line in input_data:
    ...
    if line_type == "vertical":
        ...
        plot_diagonal_line(line, matrix)
